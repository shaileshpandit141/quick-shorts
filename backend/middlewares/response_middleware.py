...
class ResponseMiddleware:
    ...
    def add_throttle_details_to_header(
        self, response: Any, throttles: List[Dict[str, Any]]
    ) -> None:
        if not throttles:
            return
    ...
    # Data Validation
    def is_data_valid(
        self, data: DataType | None, expected_keys: List[str]
    ) -> DataType:
        if not data or not isinstance(data, dict):
            self.logger.error(
                "Invalid data format: Data is missing or not a dictionary"
            )
        ...
        if sorted(data.keys()) != sorted(expected_keys):
            self.logger.error(
                f"Invalid keys: Expected {expected_keys}, got {data.keys()}"
            )
            raise InvalidDataFormatError(
