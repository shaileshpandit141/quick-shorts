...
class TokenGenerator:
    ...
    @staticmethod
    def generate(
        payload: Dict[str, Any], token_expiry_seconds=3600
        ...
        logger.debug(
            f"Generating token with salt: {token_salt} and expiry: {token_expiry_seconds}s"
        )
    ...
    @staticmethod
    def decode(
        encoded_token: str, token_salt: str, token_expiry_seconds=3600
        ...
        try:
            decoded_payload = token_serializer.loads(
                encoded_token, max_age=token_expiry_seconds
            )
