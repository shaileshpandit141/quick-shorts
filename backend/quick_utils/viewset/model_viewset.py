...
class ModelViewSet(viewsets.ModelViewSet):
    ...
    def _create_error_dict(
        self, field: str, error: Any, code: str = DEFAULT_ERROR_CODE
    ) -> Dict[str, Any]:
        ...
        return {"field": field, "code": code, "message": str(error), "details": None}
    ...
    def finalize_response(
        ...
        **kwargs,
        ...
        if request.method == "DELETE" and response.status_code == 204:
            custom_response = {"message": self.DEFAULT_DELETE_MESSAGE, "data": None}
            response.status_code = 200
        elif response.status_code >= 400:
            logger.error(f"Request failed with status code {response.status_code}")
            custom_response = (
                self._build_error_response(data)
            ...
            custom_response = {"message": self.DEFAULT_SUCCESS_MESSAGE, "data": data}
